...
%{!?rhel: %global rhel 0}
%{!?fedora: %global rhel 0}
%{!?amzn: %global amzn 0}

%{!?build_agent: %global build_agent 1}

%ifarch x86_64 aarch64
%if 0%{?rhel} >= 6 || %{amzn} >= 2023 || 0%{?fedora} > 24
%{!?build_agent2: %global build_agent2 1}
%endif
%if 0%{?rhel} >= 8 || %{amzn} >= 2023 || 0%{?fedora} > 24
%{!?build_web_service: %global build_web_service 1}
%endif
%endif

%if 0%{?rhel} >= 7 || %{amzn} >= 2023 || 0%{?fedora} > 24
...
%if 0%{?rhel} <= 5 || 0%{?rhel} >= 8 || 0%{?fedora} > 24
...
%if 0%{?rhel} >= 9 || %{amzn} >= 2023 || 0%{?fedora} > 24
...
# prep
#
# %patchN is obsolete, use %patch N (or %patch -P N): %patch1 -p1

%prep
%setup 0 -q -n %{name}-%{version}%{?alphatag}
